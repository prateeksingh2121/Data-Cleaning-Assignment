# Data-Cleaning-Assignment
Data cleaning assignment for Mann Hospitality LLP
steps I took-
Step 1-unzip the provide zip file by Python.
Step 2- Extract Summary, Payout Breakup, Order Level from Annexure Files into the target assignment file by Python.
step 3- Convert Commission Invoice pdf files to Excel by Python.
step 4- Extract Commission Invoice file from converted excel file into the target assignment file by Python.

Note:- I didn't provided Data Analyst quick assignment(target file) file to GitHub for confidentiality reason. It will be provided in Zip file via email.
